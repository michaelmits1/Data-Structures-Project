...
int bin_search(Region* list, int left, int right, int key){
    ...
    if (list[medium].cnt == key) return medium;
        ...
        return bin_search(list, left, medium - 1, key);    //search left
    }
    else if(list[medium].cnt < key){
        if(right <= left) return right+1 ;
        return bin_search(list, medium + 1, right, key);    //search right
    }
}

//  INTERPOLATION SEARCH -
int interpolation_search(Region* list, int left, int right, int key){
    if(left > right || list[left].cnt == list[right].cnt) return right + 1;
...
int bin_inter_search(Region* list, int size , int key){
    ...
    while(key != list[next].cnt ){
        ...
        if(len <= 5){
            for (int j = left; j <= right; ++j) if (list[j].cnt >= key) return j;
        }
...
//  FIND REGIONS (INTERPOLATION SEARCH) -
...
// FIND REGIONS (BINARY INTERPOLATION SEARCH) -
...
//  FIND REGIONS (BINARY INTERPOLATION SEARCH EXTENDED) -
